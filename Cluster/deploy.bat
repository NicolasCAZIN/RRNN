...
COPY "%CUDA_PATH%"\bin\cudart64_92.dll "%TRN_TEMP%"
COPY "%CUDA_PATH%"\bin\curand64_92.dll "%TRN_TEMP%"
COPY "%CUDA_PATH%"\bin\cublas64_92.dll "%TRN_TEMP%"
REM OpenCV Files
REM MATLAB Files

 REM  COPY "%TRN_TEMP%" \\KAMINO\Simulator
REM FOR %%H IN (DEATHSTAR) DO (
 FOR %%H IN (ALDERAAN,  DAGOBAH, DEATHSTAR, HOTH,KAMINO, KASHYYYK, NABOO, TATOOINE, BESPIN) DO (
